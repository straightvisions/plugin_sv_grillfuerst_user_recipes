...
final class Recipes_Middleware implements Middleware_Interface {
    ...
    public function __construct(
        ...
        $this->Api_Middleware->add([
            'route' => '/recipes/user/(?P<user_id>\d+)', // wordpress specific
            'args'  => ['methods' => 'POST', 'callback' => [$this, 'rest_get_recipes_create']]
        ]);

        // Create - Upload Images
        $this->Api_Middleware->add([
            'route' => '/recipes/(?P<recipe_id>\d+)/images/user/(?P<user_id>\d+)', // wordpress specific
            'args'  => ['methods' => 'POST', 'callback' => [$this, 'rest_get_recipes_upload_files']]
        ]);

        // Update
        $this->Api_Middleware->add([
            'route' => '/recipes/(?P<recipe_id>\d+)/user/(?P<user_id>\d+)', // wordpress specific
            'args'  => ['methods' => 'PUT', 'callback' => [$this, 'rest_get_recipes_update']]
        ]);
    ...
    // SETTER ----------------------------------------------------------------------------
    public function rest_get_recipes_create( $request ) {
        $Request = $this->Adapter->Request()->set($request);
        $user_id = $Request->getAttribute('user_id');
