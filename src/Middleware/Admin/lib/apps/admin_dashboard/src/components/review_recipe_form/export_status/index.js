...
const ExportStatus = (attributes) => {
	const [jobs, setJobs] = useState([]);
	const [exportStatus, setExportStatus] = useState([]);
	const [loading, setLoading] = useState(false);
	const fetchJobs = async () => {
		try {
			const route = routes.exportRecipe.replace('{id}', attributes.data.uuid);
	...
	const handleRestart = async () => {
		setLoading(true);
		try {
			const route = routes.exportRecipe.replace('{id}', attributes.data.uuid) + '/restart';
	...
	return (
		<div className="mt-5 bg-white px-4 py-5 shadow sm:rounded-lg sm:p-6 mb-4">
			<div className="flex justify-between items-center mb-4">
				{exportStatus === 'done' ?
					<h3 className="text-lg font-medium leading-6 text-gray-900">Export Status: <span className="text-green-700">Erfolgreich</span></h3>
					:
			...
			{ exportStatus === 'done' &&
				<a href={attributes.data.link} className="text-green-700" target="_blank">{attributes.data.link}</a>
			}
			{ exportStatus !== 'done' &&
					...
					<div>
						...
						<ul>
							{linkMediaJobs.map(job => (
								<li key={job.id}>ID: {job.id}, Title: {job.data.title}, Status: {job.status}</li>
							))}
						</ul>
					</div>
				</div>
