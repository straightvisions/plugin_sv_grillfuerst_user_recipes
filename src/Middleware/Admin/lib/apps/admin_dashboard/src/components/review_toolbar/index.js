...
export default function ReviewToolbar(props) {
	...
	const disabled = saving || submitting || publishing || refreshing || state === 'published' || state === 'draft';

	return (
		<div className="flex flex-row items-center items-stretch mb-2 gap-2">
			<button disabled={disabled  || state === 'reviewed'} onClick={onSave} type="button" className="flex items-center gap-2 px-2 py-1 bg-white border rounded text-sm border-gray-200 bg-blue-600 text-white">
				{ saving ? <> <Spinner width="4" height="4" /> Speichern </> :
					<> Speichern </>
				}
			</button>
			<button disabled={disabled || state === 'reviewed'} onClick={onSubmit} type="button" className="flex items-center gap-2 px-2 py-1 bg-white border rounded text-sm border-gray-200 bg-orange-600 text-white">
			...
			<button title="Daten vom Serer neu laden." disabled={disabled} onClick={onRefresh} type="button" className="flex items-center gap-2 px-2 py-1 bg-white border rounded text-sm border-gray-200 bg-red-600 text-white mr-0 ml-auto">
