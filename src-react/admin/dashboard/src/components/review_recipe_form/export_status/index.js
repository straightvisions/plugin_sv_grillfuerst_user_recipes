...
const ExportStatus = (props) => {
	...
	useEffect(() => {
		...
		const intervalId = setInterval(() => {
			// Only fetch if exportStatus is not 'done'
	...
	}, [attributes.data.uuid, exportStatus]);
	...
	return (
		<div className="mt-5 bg-white px-4 py-5 shadow sm:rounded-lg sm:p-6 mb-4">
			<div className="flex justify-between items-center mb-4">
				{exportStatus === 'done' ? (
					<h3 className="text-lg font-medium leading-6 text-gray-900">
						Export Status: <span className="text-green-700">Erfolgreich</span>
					</h3>
				) : (
			...
			{exportStatus === 'done' && (
				<a href={exportLink} className="text-green-700" target="_blank" rel="noopener noreferrer">
					{exportLink}
				</a>
			)}
			{exportStatus !== 'done' && (
					...
					<div>
						...
						<ul>
							{linkMediaJobs.map(job => (
								<li key={job.id}>
									ID: {job.id}, Title: {job.data.title}, Status: {job.status}
								</li>
							))}
						</ul>
					</div>
